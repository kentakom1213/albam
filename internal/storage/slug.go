...
const maxRetry = 10
const albumSlugLength = 8
const photoSlugLength = 10
...
func (s *Storage) generateUniqueAlbumSlug(tx *sql.Tx) (string, error) {
	for i := 0; i < maxRetry; i++ {
		slug, err := generateSlug(albumSlugLength)
		if err != nil {
			return "", err
		}
...
func (s *Storage) generateUniquePhotoSlug(tx *sql.Tx) (string, error) {
	for i := 0; i < maxRetry; i++ {
		slug, err := generateSlug(photoSlugLength)
		if err != nil {
			return "", err
		}
		...
		var exists bool
		err = tx.QueryRow(
			`SELECT EXISTS(SELECT 1 FROM assets WHERE slug = ?)`,
			slug,
		).Scan(&exists)
		if err != nil {
			return "", err
	...
	return "", fmt.Errorf("failed to generate unique photo slug")
}
